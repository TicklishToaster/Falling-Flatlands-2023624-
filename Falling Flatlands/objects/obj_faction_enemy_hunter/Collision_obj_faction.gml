...
if (!collision_damage_cooldown) {
	...
	if (knockback_modifier != 0) {		
		//motion_set(collision_detection_data[array_length(collision_detection_data)-1][2]-180, max(speed, 50));
		//motion_set(collision_detection_data[array_length(collision_detection_data)-1][2]-180, max(abs(speed), 2));
		motion_set(collision_detection_data[array_length(collision_detection_data)-1][2]-180, abs(speed) + 2);
		//motion_set(0, 50);
...
//if (knockback_modifier != 0) {		
//	//motion_set(collision_detection_data[array_length(collision_detection_data)-1][2]-180, max(speed, 50));
//	motion_set(collision_detection_data[array_length(collision_detection_data)-1][2]-180, 50);
//	show_debug_message("YEET")
//}