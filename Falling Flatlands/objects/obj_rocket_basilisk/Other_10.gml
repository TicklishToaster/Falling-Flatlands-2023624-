...
if (variable_instance_exists(collider, "collision_detection_data") && array_length(collider.collision_detection_data) > 0) {
	...
	var blast_list = ds_list_create();
	collision_circle_list(x, y, sprite_get_width(explosion_sprite_id), obj_faction_enemy, false, true, blast_list, false);
	for (var i = 0; i < ds_list_size(blast_list); ++i) {
		var target_id = ds_list_find_value(blast_list, i);
		if (target_id != collider.id) {
			with (target_id) {		
